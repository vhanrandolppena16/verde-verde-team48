...
function Home() {
  ...
  // Map of route paths to their respective navigation text
  const routeToText = {
    "/dashboard": "Dashboard",
    "/analysis": "Analysis",
    "/dataset": "Dataset",
  ...
  const [navigationText, setNavigationText] = useState(() => {
    const routeMatch = routeToText[window.location.pathname];
    const saved = localStorage.getItem("navigationText");
    return routeMatch || saved || "Dashboard";
  });
  const [collapsed, setCollapsed] = useState(false);

  useEffect(() => {
    const currentText = routeToText[location.pathname];
    if (currentText) {
      setNavigationText(currentText);
      localStorage.setItem("navigationText", currentText);
    }
  }, [location]);
